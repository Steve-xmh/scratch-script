...
- `blocks` 数组，必需，列出所有的模块列表，每个数组成员都是一个对象，格式如下：
    ...
    - `preprocess` 如果所有参数均为常量，那么此函数用于预计算处理，返回一个常量 AST 节点对象替换该模块。
    - `type` 模块样式类型，默认值为 `1`，可选值如下：
        ...
        - `6` ：事件模块，类似 `当 🚩 被点击` 模块
        - `7` ：多分支模块，类似 `switch`，但是在 Scratch 内是固定分支数量的。
    ...
    - `args` 模块的参数数组，每个数组成员都是一个对象，格式如下：
        - `type` 参数的类型，默认值为 `10`，可选值如下：
            ...
            - `13` 列表菜单
            - `20` 枚举菜单
        - `menu` 如果类型为枚举菜单，则此参数用于表达相应的类型，使用一个对象，键名为参数值，键值为真实值。如不提供或输入值未提供，则用户的输入值将直接作为真实值。
        - `menuOpcode` 在大多数情况下，菜单都属于独立的模块（只是因为 Shadow 所以拖不出来），所以这里如果提供，则作为这个菜单模块的模块 ID，否则将直接把菜单值存储。
